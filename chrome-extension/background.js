                ...
                if (stored.autoRecord) {
                    ...
                    if (sender.tab?.id) {
                        handleStartRecording(sender.tab.id)
                            .then(() => {
                                // Avisa content-meet.js que comecou a gravar
                                chrome.tabs.sendMessage(sender.tab.id, { type: 'recordingStarted' }).catch(() => {});
                            })
                            .catch(err => {
...
async function handleStartRecording(tabId) {
    ...
    // Obtém stream ID do tab alvo de forma garantidamente assíncrona
    const streamId = await new Promise((resolve, reject) => {
        chrome.tabCapture.getMediaStreamId({ targetTabId: tabId }, (id) => {
            if (chrome.runtime.lastError) {
                reject(new Error(chrome.runtime.lastError.message));
            } else if (!id) {
                reject(new Error('Nenhum Stream ID retornado pelo Chrome.'));
            } else {
                resolve(id);
            }
        });
