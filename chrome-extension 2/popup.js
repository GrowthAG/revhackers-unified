...
async function renderMeet(content, tab) {
  const stored = await chrome.storage.local.get(['recordingState', 'recordingStartTime', 'autoRecord']);
  ...
  document.getElementById('recBtn').addEventListener('click', async () => {
    ...
    if (isRecording) {
      ...
      isRecording = false;
      // Background must be the source of truth, but we clear it here temporarily to immediately reflect UI
      await chrome.storage.local.remove(['recordingState', 'recordingStartTime']);
      stopTimer();
      refreshMeetUI(false);
    } else {
      ...
      if (res?.success) {
        isRecording = true;
        const now = Date.now();
        // Background sets the real state
        startTimer(now);
        refreshMeetUI(true);
      } else {
