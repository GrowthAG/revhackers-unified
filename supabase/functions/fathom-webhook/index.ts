...
serve(async (req) => {
  ...
  try {
    ...
    // Achado 2026-07-18 (varredura de seguranca): a versao anterior deste
    // arquivo so existia em producao, nunca foi commitada, e so validava
    // assinatura "if (webhookSecret)" - ou seja, se o secret nunca foi
    // configurado (ou for removido), o endpoint ficava completamente
    // aberto, processando qualquer POST como evento real do Fathom (cria
    // linha em meeting_recordings e ate tarefas em orqflow_tasks com dado
    // controlado pelo chamador). Corrigido para falhar fechado: sem
    // secret configurado, a funcao rejeita tudo em vez de aceitar tudo.
    const webhookSecret = Deno.env.get('FATHOM_WEBHOOK_SECRET');
    if (!webhookSecret) {
      console.error('[fathom-webhook] FATHOM_WEBHOOK_SECRET nao configurado. Rejeitando.');
    ...
    if (signature !== webhookSecret) {
      return new Response(JSON.stringify({ error: 'Assinatura invalida' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
