...
const Sidebar = ({ isOpen, toggle }: SidebarProps) => {
    ...
    const renderMenuSection = (title: string, items: MenuItem[]) => (
        <div className="mb-6">
            {isOpen && (
                <h3 className="px-5 text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em] mb-4">
                    {title}
                </h3>
            )}
    ...
    return (
        <aside
            className={cn(
                "bg-black h-screen fixed left-0 top-0 flex flex-col border-r border-zinc-900 z-50 transition-all duration-300",
                isOpen ? "w-64" : "w-20"
            )}
        >
            <button
                        ...
                        <div className="px-2">
                            <Link
                                ...
                                className={cn(
                                    ...
                                    location.pathname === '/admin/agents' ? "bg-zinc-900 text-white" : "text-zinc-500 hover:text-white"
                                )}
                            >
                                <LayoutGrid size={16} />
                                {isOpen && <span className="text-[14px] font-bold">Explorar</span>}
                            </Link>
                        </div>

                        {isOpen && (
                            <div className="mt-6 px-5 space-y-2">
                                <h4 className="text-[10px] font-black text-zinc-700 uppercase tracking-widest">Recentes</h4>
                                {(sessions || []).slice(0, 5).map(session => (
                                    <div key={session.id} className="group relative">
                                        <Link
                                            to={`/admin/ai-chat?session=${session.id}`}
                                            className="flex items-center gap-2 py-1 text-[13px] text-zinc-500 hover:text-white transition-all truncate"
                                        >
            ...
            <div className="p-4 border-t border-zinc-900 space-y-2">
                <Link
                    to="/admin/settings"
                    className="flex items-center gap-3 px-3 py-2 text-zinc-500 hover:text-white transition-all"
                >
                ...
                <button
                    onClick={() => signOut()}
                    className="flex items-center gap-3 px-3 py-2 text-zinc-500 hover:text-red-400 transition-all w-full text-left"
                >
