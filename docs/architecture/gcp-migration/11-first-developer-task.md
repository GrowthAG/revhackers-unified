...
O caminho preferencial é o launcher local `revhackers-restricted-claude`, com Claude Code em safe mode, MCP vazio/estrito, ferramentas somente de arquivo e regras de permissão limitadas ao `dev-work`. Se Claude Code não estiver autenticado, é permitido executar Codex diretamente — nunca pelo fallback `--yolo` do CAO — com `--ignore-user-config`, `--ephemeral`, sandbox `workspace-write`, `-C` apontando somente ao `dev-work`, sem `--add-dir` e sem busca web. Reviewer e Supervisor usam sandbox `read-only`. O agente principal inspeciona diff/testes e é o único que faz commit local. Push, merge e qualquer ação externa continuam proibidos.
