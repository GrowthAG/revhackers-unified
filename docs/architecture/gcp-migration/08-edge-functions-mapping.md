# Mapeamento das 39 Supabase Edge Functions

## Escopo, método e legenda

Este inventário foi refeito sobre `5acc0ede44fd29f1806a9f0c5d4214db07725c88`. Há 40 diretórios em `supabase/functions`: `_shared` e exatamente 39 funções implantáveis. A classificação é estática; volume, duração, taxa de erro, callers externos e funções realmente implantadas não foram consultados em produção.

`supabase/config.toml` possui seção para 29 das 39 funções: 27 com `verify_jwt=true` e duas implantáveis com `false`. Dez funções não têm seção explícita e há uma seção órfã, `autentique-webhook`, sem diretório. “Gateway JWT” abaixo descreve o arquivo, não prova a configuração remota nem autorização de recurso/tenant.
...
| 3 | Reuniões, ClickUp, cron e pipelines assíncronos | filas, idempotência, sandbox, dead-letter e reconciliação |
...
| `generate-success-plan` | Gera critérios/riscos, lê/escreve planos, projetos, oportunidades e reuniões; pode atualizar ClickUp | **Tasks** | JWT ou service key validados no código → Usuário autorizador no ingresso; Workload no worker | Alto: múltiplos IDs de entrada, PII, efeito externo e estado parcial; idempotência por plano/versão |
...
## Onda 3 — reuniões, ClickUp e assíncrono
...
| `clickup-notetaker-sync` | Importa Docs do ClickUp para `meeting_recordings`; modo projeto, `sync_all` e chamada por webhook | **Scheduler → Job** para varredura; **Tasks** por projeto/evento | Gateway JWT apenas → Admin para manual; Workload para job/task | Alto: default versionado de workspace, batch, rate limit e deduplicação por doc |
| `clickup-orchestrator` | Cria folder ClickUp e persiste `clickup_integrations`/runs; declara idempotência | **Tasks** | Gateway JWT apenas → Usuário autorizado inicia; Workload executa | Alto: mutação externa e service role; validar projeto/tenant antes de enfileirar |
| `clickup-provision` | Materializa sprints/tarefas ClickUp; lê projetos/planos e registra runs | **Tasks** | Gateway JWT apenas → Admin/ação aprovada no ingresso; Workload executa | Alto: grande fan-out, retries e compensação; idempotência por projeto/template/versão |
| `clickup-sprint-orchestrator` | Cria listas/tarefas a partir de `strategic_plans`, integrações e runs | **Tasks** | Gateway JWT apenas → Usuário aprovador + Workload | Alto: side effects no ClickUp e concorrência com `clickup-provision`; decidir fonte canônica |
| `clickup-sync` | Webhook ClickUp com HMAC; grava eventos/status e pode chamar outros fluxos/GHL | **Service → Pub/Sub → Tasks** | HMAC; seção ausente; existe bypass de desenvolvimento por variável → Webhook estrito, sem bypass fora de dev isolado | Crítico: replay, ordenação, fan-out, loop de sincronização e payload; ledger por event hash |
| `clickup-update-docs-link` | Atualiza link de Docs no ClickUp e `rei_projects`/`strategic_plans` | **Tasks** | Apenas presença de header; seção ausente → Usuário com ownership no ingresso; Workload | Alto: token não é validado no código atual, IDOR e mutação externa |
...
| Função | Semântica e dependências verificadas | Mapeamento alvo | Auth atual → alvo | Risco/gate |
|---|---|---|---|---|
| `infinitepay-create-link` | Cria checkout InfinitePay e embute URL Supabase do webhook | **Service** | Gateway JWT apenas → Usuário/capability com autorização sobre proposta e valor server-side | Crítico: amount/order/redirect vêm do caller; servidor deve derivar valor, NSU e URLs aprovadas |
| `infinitepay-webhook` | Confirma pagamento, altera proposta/oportunidade, converte projeto e chama GHL/IA | **Service → Tasks** com ledger transacional | `verify_jwt=false`, segredo em header → Webhook conforme mecanismo oficial, replay guard e idempotência | Crítico: financeiro, PII em logs, muitas escritas/efeitos e idempotência parcial; divergência aceita = zero |
...
- `ai_generation_jobs`, runs do ClickUp e campos de status são embriões de ledger, mas não provam atomicidade, retry ou deduplicação completos.
- OpenAI, Anthropic, Google, ClickUp, GoHighLevel, InfinitePay, BrasilAPI e URLs arbitrárias exigem política de egress, timeout, quota e sandbox por ambiente.
